...
router.post('/postContact',(req,res,next)=>{
    ...
    let query="insert into contact (contactName, contactEmail, contactSubject, contactMessage) "+
...
router.post('/postPengaduan',(req,res,next)=>{
    ...
    let query="insert into contact (contactName, contactEmail, contactSubject, contactMessage) "+
    "values ('"+req.body.name+"','"+req.body.email+"','[Pengaduan] "+req.body.subject+"','"+req.body.message+"')";
    console.log(query);
    knex.schema.raw(query).then(ress=>{
        res.json('done');
    }).catch(err=>{
